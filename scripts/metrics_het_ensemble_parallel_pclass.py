...
python scripts/metrics_het_ensemble_parallel_pclass.py --dataset=cifar10
"""
import pandas as pd
...
from longtail_ensembles.utils_ens import read_model_size_file, get_ensembles_summary_pclass
...
def run_model(ensemble_size=4,
  ...
  random.seed(seed)
  # %% results filename
  results_filename = output_dir / out_name / ('pclass_{}'.format(dataset) + ".csv")
  ...
  if serial_run:
    ...
    for egroup_idx, ensemble_group in enumerate(tqdm(ensemble_groups)):
      result = get_ensembles_summary_pclass(
                                     file_models,
    ...
    get_ens = partial(get_ensembles_summary_pclass, file_models,
                      model_num_params, num_classes)
