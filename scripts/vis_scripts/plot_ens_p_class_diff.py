...
Figure: per class metric comparison where the metric is the average single model performance
vs the average single model loss
"""
...
def main(ind_dataset='cifar10', ood_dataset="cinic10", out_name="model_performance/het_run_v2"):
  ...
  output_filename = output_dir / "figures" / out_name / ("ens_pclass_diff_{}_{}.pdf".format(ind_dataset, ood_dataset))
  ...
  #plot_results = plot_results[plot_results['Ensemble Type'] != 'single model']
  plot_results = plot_results.sort_values(by="0-1 Error")
  #plot_results = plot_results.drop(columns=['F1'])
  ...
  all_ds = []
  class_ids = np.unique(stacked_df['Class ID'])
  for datas in ['InD','OOD']:
    for metric in ['0-1 Error']:#,'F1']:
      for class_id in class_ids:
        pv = stacked_df[np.logical_and(stacked_df['Metric'] == metric, stacked_df['Data Type'] == datas)]
        pv = pv[pv['Class ID'] == class_id]
        a_pv = pv[pv['Ensemble Type'] == 'avg. probs'].set_index('models').sort_index().reset_index()
        b_pv = pv[pv['Ensemble Type'] == 'avg. logits'].set_index('models').sort_index().reset_index()
        c_pv = pv[pv['Ensemble Type'] == 'single model'].set_index('models').sort_index().reset_index()
        # assert models are the same
        assert np.all(a_pv['models'].values == b_pv['models'].values)
        assert np.all(a_pv['models'].values == c_pv['models'].values)
        # metric gap between avg. probs and avg. logits
        f_pv = a_pv['Value'].values - b_pv['Value'].values
        offset = c_pv['Value'].values
        d_ = pd.DataFrame({
                            'models': a_pv['models'].values,
                            'Data Type': a_pv['Data Type'].values,
                            'Class ID': a_pv['Class ID'].values,
                            #'Metric': metric,
                           'no_avg':offset ,
                          'metric diff': f_pv,
                          'gain avg. probs':  a_pv['Value'].values - offset,
                          'gain avg. logits': b_pv['Value'].values - offset,
        }
                          )
        all_ds.append(d_)
  stacked_df = pd.concat(all_ds)
  stacked_df = stacked_df.set_index(['models', 'Data Type', 'Class ID','no_avg'])
  stacked_df.columns.name = 'Ensemble Type'
  stacked_df = stacked_df.stack(['Ensemble Type']).reset_index()
  #stacked_df = stacked_df.set_index(['models','Data Type','Class ID']).stack().reset_index()
  stacked_df.columns = ['models','Data Type','Class ID','no_avg','Ensemble Type','Value']

  #stacked_df.set_index(['models','Data Type','Class ID','Metric'], inplace=True)

  #stacked_df = stacked_df.set_index(['models', 'Data Type', 'Class ID'])
  #%%

  # Separate logit variance vs ensemble variance
  #%%
  metrics_columns=["0-1 Error", "F1"] #,"EnsVar","ClsVar"]
  metrics_columns=["0-1 Error"] #,"EnsVar","ClsVar"]
  ...
  model_idx = model_idx[model_idx['Class ID'] == 0]
  ...
  #%%
  # group by metric
  #stacked_df = stacked_df[stacked_df['Data Type'] == 'InD']
  colors = ['yellowgreen', 'tomato', 'dodgerblue', 'violet']
  colors = ['tomato', 'dodgerblue', 'violet']
  hue_order = ['gain avg. logits','gain avg. probs', ]#new_plot_results['Ensemble Type'].unique()
  #%%
  g = sns.relplot(
    data=stacked_df,
    x="no_avg",
  ...
  g.map_dataframe(draw_heatmap, 'Class ID', 'models', 'Value', cbar=True, square=False)
  """
  #g.set_xticklabels([])
  g.set_xlabels('')
  g.set_ylabels('')
