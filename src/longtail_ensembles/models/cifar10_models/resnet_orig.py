...
def resnet_orig(pretrained=True, device="cpu"):
    ...
    if pretrained:
        ...
        state_dict = torch.load(script_dir + "/state_dicts/resnet_orig.pt",
                                map_location=device)
        net.load_state_dict(state_dict)
    return net


def resnet_orig_110(pretrained=False, device="cpu",**kwargs):
    net = ResNet(BasicBlock, [18, 18, 18],**kwargs)
    if pretrained:
