...
from ..layers import create_subnet_params, create_subnet_params_output_only, Conv2d_subnet_layer, ChannelSwitcher, LogSoftmaxGroupLinear
import os
...
def subnetconv(params, layer):
    ...
    if layer.kernel_size == (3, 3):
        return Conv2d_subnet_layer(params,
                                   layer.in_channels,
                                   layer.out_channels,
                                   kernel_size=3,
                                   stride=layer.stride,
...
class WideResNet_GroupLinear(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = LogSoftmaxGroupLinear(512 * block.expansion * k, num_classes,
                                        k)
...
class SubResNet(nn.Module):
    ...
    def __init__(self,
        ...
        params = create_subnet_params_output_only(
            self.conv1.weight, self.nb_subnets
        )  ## returns an indexed dictionary of weight params.
        conv_subnet = subnetconv(
            params[self.indices["conv1"]],
            self.conv1)  ## create subnet. ## TODO add index
        ...
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64 * k, layers[0])
        self.replace(self.layer1,
                     self.baseresnet.layer1,
                     indices=self.indices["layer1"])
        ...
        self.layer2 = self._make_layer(block,
                                       ...
                                       stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.replace(self.layer2,
                     self.baseresnet.layer2,
                     indices=self.indices["layer2"])
        self.layer3 = self._make_layer(block,
                                       256 * k,
                                       layers[2],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.replace(self.layer3,
                     self.baseresnet.layer3,
                     indices=self.indices["layer3"])
        self.layer4 = self._make_layer(block,
                                       512 * k,
                                       layers[3],
                                       stride=2,
                                       dilate=replace_stride_with_dilation[2])
        self.replace(self.layer4,
                     self.baseresnet.layer4,
                     indices=self.indices["layer4"])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def replace(self, layerreplace, layersource, indices):
        ...
        assert len(layerreplace) == len(
            layersource
        ), "the two residual blocks contain different numbers of units. replacement impossible."
        ...
        if isinstance(layerreplace[0], BasicBlock):
            ...
            raise Exception(
                "layer is not instance of `BasicBlock` or `Bottleneck`")
        ...
        nb_layers = nb_units * layernb + int(
            replace_downsample
        )  ## add one more for the 1x1 layer if we need to downsample.
        assert nb_layers == len(
            indices), "The `indices` you passed do not cover all "
        ...
        for unit_ind in range(nb_units):
            if layertype == "Basic":
                convs = {
                    "conv1": {
                        "source": layersource[unit_ind].conv1,
                        "target": layerreplace[unit_ind]
                    },  ## source is the actual layer, target is the unit. target here is pretty worthless- you can remove later. 
                    "conv2": {
                        "source": layersource[unit_ind].conv2,
                        "target": layerreplace[unit_ind]
                    }
                }
                ...
                convs = {
                    ...
                    "conv3": {
                        "source": layersource[unit_ind].conv3,
                        "target": layerreplace[unit_ind]
                    }
                }
            ...
            for convname, convset in convs.items():
                base = convset[
                    "source"]  ## base Conv2d module we want to use for replacement.
                params = create_subnet_params(
                    base.weight, self.nb_subnets
                )  ## returns an indexed dictionary of weight params.
                conv_subnet = subnetconv(params[indices[replaceindex]],
                                         base)  ## create subnet.
                setattr(convset["target"], convname,
                        conv_subnet)  ## we have to set attribute directly.
                replaceindex += 1
            ...
            if unit_ind == 0:
                if replace_downsample:
                    ...
                    new_downsample_layers = [
                        conv_subnet, convset["target"].downsample[1]
                    ]
                    if self.alt:
...
def _widesubresnet(arch, block, layers, pretrained, progress, device, k,
                   baseresnet, indices, **kwargs):

    model = SubResNet(block,
                      layers,
                      k=k,
...
def _wideresnet_grouplinear(arch, block, layers, pretrained, progress, device,
                            k, **kwargs):
...
def _wideresnet(arch, block, layers, pretrained, progress, device, k,
                **kwargs):
...
def _resnet_regress(arch, block, layers, pretrained, progress, device,
                    **kwargs):
...
def widesubresnet18(baseresnet,
                    index,
                    pretrained=False,
                    progress=True,
                    device="cpu",
    ...
    return _widesubresnet("wideresnet18",
                          BasicBlock, [2, 2, 2, 2],
                          pretrained,
                          progress,
                          device,
...
def wideresnet18_4_grouplinear(pretrained=False,
                               progress=True,
                               device="cpu",
                               **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _wideresnet_grouplinear("wideresnet18",
                                   BasicBlock, [2, 2, 2, 2],
                                   pretrained,
                                   progress,
                                   device,
...
def wideresnet18_4(pretrained=False, progress=True, device="cpu", **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _wideresnet("wideresnet18",
                       BasicBlock, [2, 2, 2, 2],
                       pretrained,
                       progress,
                       device,
...
def resnet101(pretrained=False, progress=True, device="cpu", **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet("resnet50", Bottleneck, [3, 4, 23, 3], pretrained, progress,
                   device, **kwargs)


def resnet18_regression(pretrained=False,
                        progress=True,
                        device="cpu",
                        **kwargs):
    ...
    """
    return _resnet_regress("resnet18_regress", BasicBlock, [2, 2, 2, 2],
